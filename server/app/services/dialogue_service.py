...
class DialogueService:
    ...
    def generate_dialogue_for_npc_in_scene(self, npc_profile, scene_description, conversation_history):
        ...
        prompt_lines.append(f"Class/Role: {npc_profile.get('class', 'Unknown')}") # Ensure 'class' is loaded
        prompt_lines.append(f"Appearance: {npc_profile.get('appearance', 'Not clearly described.')}")
        
        if npc_profile.get('personality_traits'):
            prompt_lines.append(f"Key Personality Traits: {', '.join(npc_profile['personality_traits'])}. These are core to how you behave and speak.")
        else:
        ...
        if npc_profile.get('backstory'):
            prompt_lines.append(f"Relevant Backstory: {npc_profile['backstory'][:350]}...") # Slightly more context
        else:
        ...
        prompt_lines.append(f"Given your profile and the current scene, what is the one thing you, {npc_name}, say next? Your response must be only the direct quote. Do not just acknowledge the scene; react to it, ask a question, or make a statement driven by your personality, motivations, or flaws. For instance, if the GM says, \"{scene_description}\", how would YOU, {npc_name}, specifically respond with a line of dialogue? Be concise and impactful.")
        prompt_lines.append("RESPONSE:") # Added a clear marker for where the AI should start its response
        ...
        try:
            ...
            # If consistently getting safety blocks for appropriate content, you might *cautiously*
            # consider "BLOCK_ONLY_HIGH" for specific categories, but this increases risk.
            # It's better to refine prompts or ensure NPC data doesn't inadvertently trigger filters.
            safety_settings = [
            ...
            generation_config = genai.types.GenerationConfig(
                ...
                # top_p=0.95,
                # top_k=40, 
                max_output_tokens=120 # Max length for a single line of dialogue
            )
            ...
            if response.parts:
                ...
                # Cleanup: Remove potential "NPC Name:" prefix or surrounding quotes
                ...
                if len(generated_text) > 1 and ((generated_text.startswith('"') and generated_text.endswith('"')) or \
                   (generated_text.startswith("'") and generated_text.endswith("'"))):
                    # Only strip if both start and end quotes are present
                ...
                return generated_text if generated_text else f"[{npc_name} considers the situation.]" # More active fallback
            else: 
