...
def process_npc_data(npc_raw_data, npc_collection, file_name="Unknown File", is_soul_npc=False, main_npc_id_for_soul=None):
    ...
    if '_id' in npc_raw_data and npc_raw_data['_id']:
        ...
        file_name_part = os.path.splitext(file_name)[0].lower()
        base_id_prefix = f"{file_name_part}_soul_" if is_soul_npc else f"{file_name_part}_"
        
        # Use name from Vuthausk's specific structure if it's him
        if file_name.lower() == 'vuthausk.json' and not is_soul_npc and 'character_details' in npc_raw_data:
             actual_name = npc_raw_data.get('character_details', {}).get('basic_information', {}).get('name', 'vuthausk')
             npc_name_for_id_gen = actual_name.lower().replace(' ', '_').replace('-', '_')

        temp_id = f"{base_id_prefix}{npc_name_for_id_gen}"
        
        # For souls, their name might be long, so truncate for ID generation if necessary
        if is_soul_npc and len(npc_name_for_id_gen) > 20: # Arbitrary length
            npc_name_for_id_gen = npc_name_for_id_gen[:20]

        # Ensure some uniqueness for generated IDs, especially for generic names or souls
        # A simple count or UUID might be needed if names are not unique enough
        # For now, let's assume this is mostly okay for initial load, but flag it.
        existing_doc = npc_collection.find_one({"_id": temp_id})
        if existing_doc and existing_doc.get('name') != npc_raw_data.get('name'): # If ID exists but for different NPC
            npc_id = f"{temp_id}_{str(uuid.uuid4())[:4]}"
            print(f"Warning [{file_name}]: Generated _id '{temp_id}' might conflict. Appended UUID part: '{npc_id}'")
        else:
            npc_id = temp_id

        npc_id_source = "generated"
        print(f"Info [{file_name}]: NPC '{npc_raw_data.get('name', 'Unnamed')}' using _id '{npc_id}' (source: {npc_id_source})")
    ...
    npc_class = npc_raw_data.get('class') # 'class' is a keyword, so use npc_class

    # Special handling for Vuthausk's main entry from Vuthausk.json
    if file_name.lower() == 'vuthausk.json' and not is_soul_npc and 'character_details' in npc_raw_data:
        details = npc_raw_data.get('character_details', {})
        basic_info = details.get('basic_information', {})
        personality_backstory = details.get('personality_and_backstory', {})

        name = basic_info.get('name', name)
        race = basic_info.get('race', race)
        npc_class = basic_info.get('class', npc_class)
        # Construct a more detailed appearance for Vuthausk
        ...
        personality_traits = personality_backstory.get('personality_traits', personality_traits)
        backstory = personality_backstory.get('backstory', backstory)
        motivations = personality_backstory.get('motivations', motivations)
        flaws = personality_backstory.get('flaws', flaws)
        # You can add more specific fields here:
        # 'ideals': personality_backstory.get('ideals'),
        # 'bonds': personality_backstory.get('bonds'),
        # 'speech_patterns': details.get('speech_and_mannerisms', {}).get('speech_patterns')
    ...
    npc_doc = {
        ...
        'flaws': flaws,
        'race': race,
        'class': npc_class, # Storing as 'class' in DB
        'source_file': file_name,
        'is_soul_npc': is_soul_npc,
        'main_npc_id_if_soul': main_npc_id_for_soul if is_soul_npc else None,
    }
    npc_doc = {k: v for k, v in npc_doc.items() if v is not None and v != '' and (isinstance(v, list) and len(v) > 0 or not isinstance(v, list))}
    ...
    try:
        ...
        if result.upserted_id:
            ...
            print(f"Warning [{file_name}]: NPC _id '{npc_doc['_id']}' - No upsert/match. Result: {result.raw_result}")
            skipped = 1
            
    except errors.DuplicateKeyError:
        print(f"MongoDB DuplicateKeyError for _id '{npc_doc['_id']}' from {file_name}. Skipping.")
        skipped = 1
    except errors.PyMongoError as e:
        print(f"MongoDB error for _id '{npc_doc['_id']}' from {file_name}: {e}. Skipping.")
        skipped = 1
    except Exception as e:
        print(f"Unexpected error for _id '{npc_doc['_id']}' from {file_name}: {e}. Skipping.")
        skipped = 1
...
def load_npcs_to_db():
    ...
    if not json_files:
        print(f"No .json files found in {NPC_DATA_DIR_PATH}.")
        return

    print(f"Found JSON files: {', '.join(os.path.basename(f) for f in json_files)}")
    ...
    for json_file_path in json_files:
        ...
        npcs_to_process = []
        vuthausk_main_id_for_souls = None # To link souls to Vuthausk's processed _id
        ...
        if file_name.lower() == 'vuthausk.json' and isinstance(data_from_file, dict) and 'character_details' in data_from_file:
            ...
            # Vuthausk himself
            # His _id will be determined in process_npc_data based on his name
            ...
            # Generate a predictable base ID for Vuthausk to link souls, even if process_npc_data slightly alters it with UUID for collision
            vuthausk_main_id_for_souls = f"vuthausk_{vuthausk_name_from_json.lower().replace(' ', '_')}"
            ...
            if isinstance(souls_list, list) and souls_list:
                ...
                for soul_str in souls_list:
                    if isinstance(soul_str, str) and soul_str.strip():
                        soul_name = soul_str.split(' - ')[0].strip()
                        soul_npc_data = {
                            'name': soul_name if soul_name else "Unnamed Soul",
                            'appearance': f"A past life soul: {soul_str}",
                            'personality_traits': ["Past Life Influence"],
                            # _id will be generated in process_npc_data
                        }
        ...
        for i, npc_raw in enumerate(npcs_to_process):
            ...
            if file_name.lower() == 'vuthausk.json':
                if i == 0: # This is Vuthausk himself
                    is_soul = False
                else: # These are the souls
                    is_soul = True
                    main_id_link = vuthausk_main_id_for_souls # Link souls to Vuthausk's base ID
    ...
    print(f"NPC records in files: {total_records_in_files}")
